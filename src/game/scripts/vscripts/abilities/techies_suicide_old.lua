...
LinkLuaModifier("modifier_techies_custom_suicide_stun", "abilities/techies_suicide_old.lua", LUA_MODIFIER_MOTION_NONE) -- for shard, needs tooltip
LinkLuaModifier("modifier_techies_custom_blast_off", "abilities/techies_suicide_old.lua", LUA_MODIFIER_MOTION_BOTH)
	...
	for _, enemy in pairs(enemies_big_radius) do
		if enemy and not enemy:IsNull() then
			...
			if has_shard and not enemy:IsBuilding() and not enemy:IsMagicImmune() then
				enemy:AddNewModifier(caster, self, "modifier_techies_custom_suicide_stun", {duration = stun_duration})
			end
