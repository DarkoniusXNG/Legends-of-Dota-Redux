--------------------------------------------------------------------------------------------------------
--		Hero: Bane
--		Perk: Bane gains 1% Spell lifesteal for each level put in a Demonic or Sleeping ability. Bonus 150% Spell lifesteal against sleeping units.
--------------------------------------------------------------------------------------------------------
...
function modifier_npc_dota_hero_bane_perk:OnIntervalThink()
	if IsServer() then
		local caster = self:GetParent()
		local stacks = 0
		for i = 0, caster:GetAbilityCount() - 1 do
			local skill = caster:GetAbilityByIndex(i)
			if skill and (skill:HasAbilityFlag("demon") or skill:HasAbilityFlag("sleep")) then
...
function modifier_npc_dota_hero_bane_perk:DeclareFunctions()
	return {
...
if IsServer() then
  function modifier_npc_dota_hero_bane_perk:OnTakeDamage(event)
    ...
    local heal_amount = damage * total_spell_lifesteal * 0.01
