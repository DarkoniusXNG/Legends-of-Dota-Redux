-- Splasher
LinkLuaModifier("modifier_any_damage_splash_oaa", "abilities/oaa/splasher.lua", LUA_MODIFIER_MOTION_NONE)

splasher_oaa = class({})

function splasher_oaa:GetIntrinsicModifierName()
	return "modifier_any_damage_splash_oaa"
end

---------------------------------------------------------------------------------------------------
modifier_any_damage_splash_oaa = class({})
...
if IsServer() then
  ...
  function modifier_any_damage_splash_oaa:Splash(damage, dmg_flags, dmg_type, damaged_unit, inflictor)
    ...
    if damage > 0 and #targets > 1 then
      local particle = ParticleManager:CreateParticle("particles/items/powertreads_splash.vpcf", PATTACH_POINT, damaged_unit) -- TODO: Add particle
