-- Sorcerer (chance to refresh on cast or kill)
LinkLuaModifier("modifier_sorcerer_oaa", "abilities/oaa/sorcerer.lua", LUA_MODIFIER_MOTION_NONE)

sorcerer_oaa = class({})

function sorcerer_oaa:GetIntrinsicModifierName()
	return "modifier_sorcerer_oaa"
end

---------------------------------------------------------------------------------------------------

modifier_sorcerer_oaa = class({})
...
if IsServer() then
  function modifier_sorcerer_oaa:OnAbilityFullyCast(event)
    ...
    if RandomInt(1, 100) <= chance then
      if not no_charges and not IsFakeItemCustom(ability) then -- TODO: IsFakeItemCustom
  ...
  function modifier_sorcerer_oaa:OnHeroKilled(event)
    ...
    if target:IsClone() or target:IsTempestDouble() or target:IsSpiritBearCustom() then
      return
    end
    ...
    if RandomInt(1, 100) <= self.chance_for_abilities_on_kill then
      ...
      for i = 0, parent:GetAbilityCount() - 1 do
        ...
        if ability and AllowedToRefresh(ability, true) then -- AllowedToRefresh
          if not IsFakeItemCustom(ability) then -- TODO: IsFakeItemCustom
            ability:RefreshCharges()
          end
          ability:EndCooldown()
        end
