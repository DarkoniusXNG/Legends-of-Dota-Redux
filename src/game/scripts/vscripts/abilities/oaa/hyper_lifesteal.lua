-- Hyper Lifesteal
LinkLuaModifier("modifier_any_damage_lifesteal_oaa", "abilities/oaa/hyper_lifesteal.lua", LUA_MODIFIER_MOTION_NONE)

hyper_lifesteal_oaa = class({})

function hyper_lifesteal_oaa:GetIntrinsicModifierName()
	return "modifier_any_damage_lifesteal_oaa"
end

---------------------------------------------------------------------------------------------------

modifier_any_damage_lifesteal_oaa = class({})
...
if IsServer() then
	function modifier_any_damage_lifesteal_oaa:OnTakeDamage(event)
		...
		-- Ignore self damage - WORKS ON SELF DAMAGE!
		...
		local spellLifestealReflected = true --false - WORKS ON REFLECTED DAMAGE!
		--if isSuccubus then
			--spellLifestealReflected = succubus:GetSpecialValueFor("lifesteal_reflected") == 1
		--end

		-- Ignore pure damage - WORKS ON PURE DAMAGE!
