pudge_flesh_heap_int = class({})

LinkLuaModifier("modifier_flesh_heap_int", "abilities/pudge_flesh_heap_int.lua", LUA_MODIFIER_MOTION_NONE)

function pudge_flesh_heap_int:Spawn()
	if IsServer() then
		local caster = self:GetCaster()
		-- Add kill tracker modifier
		if not caster:HasModifier("modifier_pudge_custom_flesh_heap_kill_tracker") then
			caster:AddNewModifier(caster, self, "modifier_pudge_custom_flesh_heap_kill_tracker", {})
		end
	end
end

function pudge_flesh_heap_int:GetIntrinsicModifierName()
  return "modifier_flesh_heap_int"
end

---------------------------------------------------------------------------------------------------

modifier_flesh_heap_int = class({})

function modifier_flesh_heap_int:IsHidden()
    return false
end

function modifier_flesh_heap_int:IsDebuff()
	return false
end

function modifier_flesh_heap_int:IsPurgable()
    return false
end

function modifier_flesh_heap_int:RemoveOnDeath()
    return false
end

function modifier_flesh_heap_int:OnCreated()
	local parent = self:GetParent()
	local ability = self:GetAbility()

	if not ability or ability:IsNull() then
		self:Destroy()
		parent:CalculateStatBonus(true)
		return
	end

	self.flesh_heap_amount = ability:GetSpecialValueFor("flesh_heap_intelligence_buff_amount") or 0
	if IsServer() then
		local stacks = parent:GetModifierStackCount("modifier_pudge_custom_flesh_heap_kill_tracker", parent)
		self:SetStackCount(stacks)
		parent:CalculateStatBonus(true)
	end
end

function modifier_flesh_heap_int:OnRefresh()
	self:OnCreated()
end

function modifier_flesh_heap_int:DeclareFunctions()
  return {
    MODIFIER_PROPERTY_STATS_INTELLECT_BONUS,
  }
end

function modifier_flesh_heap_int:GetModifierBonusStats_Intellect()
  local parent = self:GetParent()
  return parent:GetModifierStackCount("modifier_pudge_custom_flesh_heap_kill_tracker", parent) * self.flesh_heap_amount
end
