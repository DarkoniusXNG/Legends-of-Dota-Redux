LinkLuaModifier("modifier_bubble_witch_cavitation_movement", "abilities/aba/bubble_witch_cavitation.lua", LUA_MODIFIER_MOTION_HORIZONTAL)
LinkLuaModifier("modifier_bubble_witch_cavitation_debuff", "abilities/aba/bubble_witch_cavitation.lua", LUA_MODIFIER_MOTION_NONE) -- needs tooltip
...
if IsServer() then
  function modifier_bubble_witch_cavitation_debuff:OnDestroy()
    ...
    if parent and not parent:IsNull() and parent:IsAlive() then
      ...
      EmitSoundOnLocationWithCaster(parent_origin, "n_frogs.WaterBubble.Destroy", caster)
    end
  end
end
