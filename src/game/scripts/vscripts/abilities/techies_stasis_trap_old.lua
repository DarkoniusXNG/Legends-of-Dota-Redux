...
LinkLuaModifier("modifier_techies_custom_stasis_trap", "abilities/techies_stasis_trap_old.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_techies_custom_stasis_trap_stun", "abilities/techies_stasis_trap_old.lua", LUA_MODIFIER_MOTION_NONE) -- needs tooltip
					...
					if #enemies_again > 0 then
						...
						for _, enemy in pairs(enemies_again) do
							if enemy and not enemy:IsNull() and not enemy:IsMagicImmune() then
								...
								enemy:AddNewModifier(parent, ability, "modifier_techies_custom_stasis_trap_stun", {duration = stun_duration})
							end
						end
