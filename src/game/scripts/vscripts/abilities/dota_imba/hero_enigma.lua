...
function modifier_imba_enigma_eidolon:OnCreated( keys )
	...
	self.attack_speed_bonus	= (self.parent:GetAttacksPerSecond(false) * self.parent:GetBaseAttackTime() * 100) * self.trans_pct * 0.01
	...
	if self.parent:HasModifier("modifier_imba_echo_rapier_haste") then
		...
		self.attack_speed_bonus = ((self.parent:GetAttacksPerSecond(false) * self.parent:GetBaseAttackTime() * 100) - echo_buf.attack_speed_buff) * self.trans_pct * 0.01
	end
...
function imba_enigma_black_hole:OnChannelFinish( bInterrupted )
	...
	if bInterrupted then		
		...
		Timers:CreateTimer(FrameTime(), function()
			if caster:IsRooted() or caster:IsLeashedCustom() or caster:IsSilenced() or caster:IsStunned() or caster:IsHexed() or caster:IsCommandRestricted() or not caster:IsAlive() then
