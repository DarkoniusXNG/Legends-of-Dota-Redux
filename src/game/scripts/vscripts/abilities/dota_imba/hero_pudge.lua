...
--	self:GetCaster():StartGesture(ACT_DOTA_CHANNEL_ABILITY_4)
	...
	self.target = target
	self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_imba_pudge_dismember_buff", {target_index = target:GetEntityIndex()})
...
function modifier_imba_pudge_dismember_buff:OnCreated(kv)
	local ability = self:GetAbility()
	if ability and not ability:IsNull() then
		self.spell_lifesteal = ability:GetSpecialValueFor("spell_lifesteal")
		self.range = 200 --ability:GetCastRange(nil, nil)
	end
	self.spell_lifesteal_penalty_against_creeps = 80

	if IsServer() then
		self:SetStackCount(kv.target_index)
	end
end

if IsServer() then
	function modifier_imba_pudge_dismember_buff:OnTakeDamage(event)
		local parent = self:GetParent()
		local ability = self:GetAbility()
		...
		if not self.spell_lifesteal_penalty_against_creeps then
			self.spell_lifesteal_penalty_against_creeps = 80
		end
...
function modifier_imba_pudge_dismember_buff:DeclareFunctions()
	local table = {
		...
		-- elfansoer: fix spell lifesteal not implemented
...
function modifier_imba_pudge_dismember_buff:GetActivityTranslationModifiers()
	local parent = self:GetParent()
	local target_index = self:GetStackCount()
	local long_range = false
	if target_index and target_index ~= 0 then
		...
		if target and not target:IsNull() then
			...
			long_range = distance > self.range
		end
	end
	if parent:HasItemInInventory("item_imba_aether_lens") or long_range then
