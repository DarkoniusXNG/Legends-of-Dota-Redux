LinkLuaModifier("modifier_axe_custom_cut_from_above_leap", "abilities/wraith_night/cut_from_above.lua", LUA_MODIFIER_MOTION_BOTH)
LinkLuaModifier("modifier_axe_custom_cut_from_above_buff", "abilities/wraith_night/cut_from_above.lua", LUA_MODIFIER_MOTION_NONE) -- needs tooltip
...
function axe_custom_cut_from_above:OnSpellStart()
  ...
  caster:AddNewModifier(caster, self, "modifier_axe_custom_cut_from_above_leap", {
...
modifier_axe_custom_cut_from_above_leap = class({})

function modifier_axe_custom_cut_from_above_leap:IsHidden()
  return true
end

function modifier_axe_custom_cut_from_above_leap:IsDebuff()
  return false
end

function modifier_axe_custom_cut_from_above_leap:IsPurgable()
  return false
end

function modifier_axe_custom_cut_from_above_leap:GetPriority()
  return DOTA_MOTION_CONTROLLER_PRIORITY_HIGHEST
end

function modifier_axe_custom_cut_from_above_leap:DeclareFunctions()
  return {
...
function modifier_axe_custom_cut_from_above_leap:GetModifierDisableTurning()
  return 1
end

function modifier_axe_custom_cut_from_above_leap:GetOverrideAnimation()
  return ACT_DOTA_CAST_ABILITY_4
end

function modifier_axe_custom_cut_from_above_leap:GetOverrideAnimationRate()
  return 0.833
end

function modifier_axe_custom_cut_from_above_leap:CheckState()
  return {
...
if IsServer() then
  function modifier_axe_custom_cut_from_above_leap:OnCreated(keys)
  ...
  function modifier_axe_custom_cut_from_above_leap:OnHorizontalMotionInterrupted()
    self:Destroy()
  end

  function modifier_axe_custom_cut_from_above_leap:OnVerticalMotionInterrupted()
    self:Destroy()
  end

  function modifier_axe_custom_cut_from_above_leap:UpdateHorizontalMotion(parent, delta_time)
    parent:SetAbsOrigin(parent:GetAbsOrigin() + Vector(self.x_tick, self.y_tick, self.jump))
  end

  function modifier_axe_custom_cut_from_above_leap:UpdateVerticalMotion(parent, delta_time)
  ...
  function modifier_axe_custom_cut_from_above_leap:OnDestroy()
    ...
    for _, unit in pairs(units) do
      if unit and not unit:IsNull() then
        if unit:GetHealth() <= damage * (1 + (caster:GetSpellAmplification(false) * 0.01)) then
          -- Remove modifiers that prevent Death
          unit:DispelDeathPreventingBuffs()
          -- Kill the unit
